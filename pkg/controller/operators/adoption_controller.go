...
var (
// TODO(njhale): factor out free variables
)

// AdoptionReconciler finds and associates Operator components.
type AdoptionReconciler struct {
	...
	// operators contains the names of Operators the AdoptionReconciler has observed exist.
...
func (r *AdoptionReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&source.Kind{Type: &operatorsv1alpha1.Subscription{}}, &handler.EnqueueRequestForObject{}).
...
var _ reconcile.Reconciler = &AdoptionReconciler{}

func (r *AdoptionReconciler) reconcileSubscription(req ctrl.Request) (ctrl.Result, error) {
	...
	// OLM generated Operators are named after their packages
	if in.Spec == nil || in.Spec.Package == "" {
		log.Info("subscription missing status, ignoring")
	}

	// Wrap with convenience decorator
	operator, err := r.factory.NewPackageOperator(in.Spec.Package)
	if err != nil {
...
func (r *AdoptionReconciler) updateComponents(ctx context.Context, operator *decorators.Operator) error {
...
func (r *AdoptionReconciler) listComponents(ctx context.Context, selector labels.Selector) ([]runtime.Object, error) {
...
func (r *AdoptionReconciler) observed(name types.NamespacedName) bool {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *AdoptionReconciler) observe(name types.NamespacedName) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.operators[name] = struct{}{}
}

func (r *AdoptionReconciler) unobserve(name types.NamespacedName) {
	r.mu.Lock()
	defer r.mu.Unlock()
	delete(r.operators, name)
}

func (r *AdoptionReconciler) mapComponentRequests(obj handler.MapObject) (requests []reconcile.Request) {
	if obj.Meta == nil {
		return
	}
