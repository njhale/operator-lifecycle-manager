...
// NewSchemedOperatorFactory adds the required operator discovery types to the given scheme and returns an OperatorFactory that
// supplies it to all Operators it creates.
func NewSchemedOperatorFactory(scheme *runtime.Scheme) (OperatorFactory, error) {
	...
	if err := operatorsv2alpha1.AddToScheme(scheme); err != nil {
		return nil, err
	}
