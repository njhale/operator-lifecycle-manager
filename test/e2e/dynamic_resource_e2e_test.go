...
func TestDynamicResourcesResolvePrometheusAPI(t *testing.T) {
	...
	_, err = dynamicClient.Resource(gvr).Namespace(ns.GetName()).Get("my-prometheusrule", metav1.GetOptions{})
	require.NoError(t, err)

	gvr.Resource = "servicemonitors"
	_, err = dynamicClient.Resource(gvr).Namespace(ns.GetName()).Get("my-servicemonitor", metav1.GetOptions{})
	require.NoError(t, err)
}
